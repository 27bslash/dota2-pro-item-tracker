...
def create_item_description(desc, datamined_items, key):
    ...
    for string in desc:
        ...
        if not re.search('--', string):
            string = re.sub(r"([A-Z][a-z]+\s(?=[a-z0-9%]))",
                            '-- \\1',  string, 1)
        ...
        for word in hint_vals:
            ...
            if 'AbilityValues' in item:
                ...
                if word and word.lower() in lowercase_vals:
                    ...
                    repl = special_values[d_key] if type(
                        special_values[d_key]) == str else special_values[d_key]['value']
                    string = re.sub(
                        fr"%+{word}%?%", repl, string)
        res.append(string)
...
def update_item_attributes(desc, datamined_items, all_abilities, key):
    # desc = ["Active: Swift Blink Teleport to a target point up to %blink_range%%% units away.After teleportation, you gain %bonus_movement%%% phased movement speed and +%bonus_agi_active% Agility for %duration% seconds. Swift Blink cannot be used for %blink_damage_cooldown% seconds after taking damage from an enemy hero or Roshan."]
    ...
    for stat in attrib:
        ...
        attrib_dict = {'key': stat, 'header': symbol,
                       'value': value, 'footer': footer, }
...
def update_json_data():
    ...
    all_abilities = json.loads(requests.get(all_abilities_url).text)[
        'lang']['Tokens']
    all_abilities = {re.sub('DOTA_Tooltip_Ability_', 'DOTA_Tooltip_ability_', k): v
    ...
    odota_items(odota_items_json,  datamined_items, all_abilities)
