...
def dl_dota2_abilities(manual=False):
    ...
    hero_list = db['hero_list'].find_one({})
    if not hero_list:
        return
    for hero in hero_list['heroes']:
        ...
        talents = soup.find_all(
            'div',  attrs={'class', re.compile('heropage_TalentEntry')})[0:8:1]
...
def update_talents():
    ...
    for hero in hero_list['heroes']:
        ...
        db['talents'].find_one_and_update(
            {'hero': switcher(hero['name'])}, {'$set': {'talents': short_talents}}, upsert=True)
        # print(talents)
...
def extract_special_values(talents, hero):
    for k in talents:
        ...
        try:
            ...
            print('Exception: ', hero, k,
                  talents[k]['name_loc'], traceback.format_exc())
    db['hero_stats'].find_one_and_update(
        {'hero': hero},
        {'$set': {'talents': talents}
         }, upsert=True)
