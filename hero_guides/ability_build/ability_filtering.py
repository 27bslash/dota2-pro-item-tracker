...
def ability_filter(data, talent_build):
    abilities = []
    all_abilities = [
        ability['img'] for match in data for ability in match['abilities'] if ability['type'] == 'ability']
    # print(len(all_abilities))
    non_ults = set()
    min_ult_level = 5 if 'meepo' not in all_abilities[0] else 3
    for ability in [ability for match in data for ability in match['abilities'] if ability['type'] == 'ability']:
    ...
    for match in data:
        ...
        for ability in match['abilities']:
            if ability['type'] != 'ability':
                continue
            d[ability['img']] = {
                'count': d[ability['img']]['count'] + 1, 'level': ability['level']} if ability['img'] in d else {'count': 1, 'level': ability['level']}
            if d[ability['img']]['count'] == 2:
                if ability['img'] in second_occurance:
                    ...
                    second_occurance[ability['img']] = {'count': second_occurance[ability['img']]
                                                        ['count'] + 1, 'level': level_list}
                else:
                    second_occurance[ability['img']] = {
                        'count': 1, 'level': [ability['level']]}
    meidans = [{k: statistics.median(second_occurance[k]['level'])}
               for k in second_occurance]
    ...
    if len(ability_count) > 3:
        try:
            ult = [ability[0]
                   for ability in ability_count if ability[0] not in non_ults][0]
    ...
    for match in data:
        if not 'abilities' in match:
            continue
        ability_string = '__'.join([ability['img']
                                    for ability in match['abilities'] if ability['type'] == 'ability'][: 10])
        abilities.append(ability_string)
    count = count_occurences(abilities)[0]
    # print(count)
    # print(talent_build)
    ret = fill_abilities(count, talent_build, ult=ult, medians=meidans)
    return ret
...
def fill_abilities(count: tuple, talents: list, ult, medians: dict):
    ...
    while (True):
        for i, ability in enumerate(ability_count):
            ...
            if ability[0] not in list(medians.keys()):
                continue
            if ability[1] < max_level and ability[0] != ult and _i+1 > medians[ability[0]]-1:
                if not ret[_i]:
                    ability[1] += 1
        ...
        non_ult_count = all(
            x[1] >= max_level for x in ability_count if x[0] != ult)
        if non_ult_count:
