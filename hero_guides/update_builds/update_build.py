...
def update_build(hero: str, role: str, data: list):
    ...
    filtered_data = [
        ...
        for match in data
        if match["role"] == role
        or match["role"] in combined_roles
        and role in combined_roles
    ]
    ...
    starting_items = [{f"item_{i}": item} for i, item in enumerate(starting_items[0])]
    ...
    ability_build = ability_filter(
        filtered_data, talent_build, count=None, build_data=filtered_data
    )
    ...
    ability_build = {"ability_build": ability_build, "abilityTooltips": talent_tooltips}
