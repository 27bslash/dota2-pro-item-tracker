...
def item_get(hero_name):
    ...
    data = hero_output.find({'hero': hero_name})
    ...
    if check_response:
        ...
        return render_template(template, max=max_val, most_used=most_used, hero_img=clean_name(hero_name), display_name=display_name, hero_name=hero_name, data=match_data,
                               time=time.time(), total=total, role_total=len(match_data), talents=get_talent_order(match_data, hero_name), hero_colour=get_hero_name_colour(hero_name), roles=roles)
    else:
